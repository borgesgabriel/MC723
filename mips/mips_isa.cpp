...
struct variables
{
    ...
    static constexpr bool is_forwarding = true;
    ...
    variables(): number_of_instructions(0), number_of_hazards(0)
    {
        last_write.resize(34);
        hazard_table = {{1, 1, 1}, {1, 1, 1}};
    }
    ...
    void write_hazard(mips_instruction inst)
    {
        ...
        if (inst.op == 0x18 || inst.op == 0x19 || inst.op == 0x1A || inst.op == 0x1B)
        {
    ...
    void read_hazard(mips_instruction inst)
    {
        ...
        std::cout << "Number of hazards: " << number_of_hazards << std::endl;
    }
